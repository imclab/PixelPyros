		...
		while(it!=connectedPhones->end()) { 
			...
			if((phone!=NULL) && ((!onlyUseFoundPhones) || phone->found)){
				...
				if(nyanCatchPhonesByConnectedPhone.find(phone) == nyanCatchPhonesByConnectedPhone.end()) { 
                    ...
                    nyanPhone->screenUnitPos.set(phone->unitPosition); 
	...
	// else if we're playing 	
		...
		while(it!=connectedPhones->end()) { 
			...
			if(nyanCatchPhonesByConnectedPhone.find(cphone) != nyanCatchPhonesByConnectedPhone.end()) { 
				...
				if((phone->displayCatTime - timer > 0) && (phone->displayCatTime - timer < catInterval)) { 
...
void NyanCatch :: draw(ofRectangle* drawRect) { 
	...
	if(updatePhysics) {
		if(currentPhone) catTargetPos.set(drawRect->x + currentPhone->screenUnitPos.x*drawRect->width,drawRect->y + currentPhone->screenUnitPos.y*drawRect->height); 
		catVel*= 0.8; 
		
		catVel += (catTargetPos-catPos)*0.05; 
		catPos+=catVel; 
...
bool nyanCatchSortFunction(NyanCatchPhone * first, NyanCatchPhone * second) { 
    ...
    int rowfirst = floor(first->screenUnitPos.y*10);
    int rowsecond = floor(second->screenUnitPos.y*10); 
    float index1 = rowfirst + ((rowfirst%2==0) ? first->screenUnitPos.x : 1-first->screenUnitPos.x);
    float index2 = rowsecond + ((rowsecond%2==0) ? second->screenUnitPos.x : 1-second->screenUnitPos.x);
	...
	while(it!=connectedPhones->end() ) {
		...
		if((!onlyUseFoundPhones) || (it->second->found)) {
			...
			if(nyanCatchPhonesByConnectedPhone.find(it->second)!=nyanCatchPhonesByConnectedPhone.end()) {
                ...
                phone->screenUnitPos.set(it->second->unitPosition); 
