...
void MemoryGame::updateWinners() { 
	...
	// SERIOUSLY, what were you thinking? These phones could disconnect 
	// at any time and you've got a null pointer. Just make a custom data 
	// object you lazy shithead. 
			...
			while(cphone->programMessages.size()>0) {
				...
				if(msg.find("press")!=string::npos) {
					...
					if(values.size()>4) { 
						...
						int level = ofToInt(values[4]); 
						
						if(data->inPlay && !data->levelSuccess && (levelNum==level)) {
							
							
							cout << startReceivingTime << " " << stopReceivingTime << "\n";
							cout << "button press " << button << " at time "<< time << " level "<< level << "\n";
							
							
							if((time>startReceivingTime) && (time<stopReceivingTime)) { 
								data->addButtonPress(level, button, time-startReceivingTime);
								
								
							} 	
							int numCorrect = data->checkButtonPresses(&colours, level);; 
							
							// THIS IS WHERE I GOT AN EXCEPTION - I'm guessing
							// borked data obejct. 
...
void MemoryGame::newLevel(){ 
	...
	startShowingTime = ofGetElapsedTimeMillis()+2000; 

	sendAllPhones("p:mem|level|"+ofToString(levelNum));
