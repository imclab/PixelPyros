...
void PhoneRenderer :: updatePhonesWithFBO( ofFbo * fbo) { 
	...
	while(phoneit!=phones->end()) {
		...
		if((!onlyUseFoundPhones) || phone->found) {
			...
			ofColor col = pixels.getColor(ofMap(phone->warpedPosition.x,0,1,0, fbo->getWidth()-1,true), ofMap(phone->warpedPosition.y,0,1,0, fbo->getHeight()-1,true)); 
...
void PhoneRenderer :: clearPhones() { 
	...
	while(phoneit!=phones->end()) {
		...
		phone->sendColour(0,latency,0,0);
